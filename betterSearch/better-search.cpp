...
static void betterSearch(const AlignedIntArray &hayStack, const AlignedIntArray & needles, AlignedIntArray &indices, StackAllocator &/*allocator*/) {
        ...
        for (int c = 0; c < needles.getCount(); c++) {
            const int value = needles[c];
            indices[c] = -1; // default. when not found, next iteration

            if (hayStackCount == 0) continue;

            int low = 0;
            int high = hayStackCount - 1;

            // out-of-range values
            if (value < hayStack[low] || value > hayStack[high]) continue;

            while (low <= high) {
                // Handle case where all remaining elements are identical
                if (hayStack[low] == hayStack[high]) {
                    if (hayStack[low] == value) {
                        indices[c] = low;
                    }
                    break;
                }
                ...
                double pos = low + ((double)(value - hayStack[low]) * (high - low)) /
                            (hayStack[high] - hayStack[low]);

                // Ensure position stays within current bounds
                int probe = static_cast<int>(pos);
                probe = std::max(low, std::min(high, probe));

                // Check if we found the value
                if (hayStack[probe] == value) {
                    // Linear search backward to find first occurrence (if duplicates exist)
                    while (probe > low && hayStack[probe - 1] == value) {
                        probe--;
                    }
                    indices[c] = probe;
                    break;
...
int main() {
    ...
    for (int r = 0; r < testCaseCount; r++) {
        ...
        const int testRepeat = std::min<int64_t>(5ll, searches / hayStack.getCount()); // modified to 5, coz its slow
